...
struct /*HPG_EXPORT*/ VisibilityGridder final {
  ...
  kernel(
    ...
    auto shmem_size = scratch_phscr_view::shmem_size(max_cf_size_y);
...
struct /*HPG_EXPORT*/ VisibilityGridder<N, execution_space, 1> final {
  ...
  using vg = VisibilityGridder<N, execution_space, 0>;

  using member_type = typename vg::member_type;

  using scratch_phscr_view = typename vg::scratch_phscr_view;

  static KOKKOS_FORCEINLINE_FUNCTION bool
  do_process(
    const Vis<N, execution_space>& vis,
    const K::Array<int, 2>& grid_size,
    const K::Array<int, 2>& cfsz_range) {

    // process this visibility only if its CF size falls within the range
    // cfsz_range and all of the updated grid points are within grid bounds
    const auto cfsz = std::min(vis.m_cf_size[0], vis.m_cf_size[1]);
    return
      (cfsz_range[0] < cfsz)
      && (cfsz <= cfsz_range[1])
      && (0 <= vis.m_grid_coord[0])
  ...
  /** parallel_for that adapts thread team size to CF kernel size
   *
   * Compute a sequence of thread team sizes for all CF kernel sizes between
   * min_cf_size and max_cf_size, then iterate through that sequence of team
   * sizes and execute Kokkos::parallel_for() with a kernel that acts only on
   * visibilities with a CF kernel size that lies within a given range. The
   * range of kernel sizes for each value of team size is computed by this
   * function. The caller must ensure that f acts only on visibilities that are
   * associated with a CF kernel size in the given range; the interpretation of
   * "CF kernel size" is arbitrary, but generally the "CF kernel size" of every
   * visibility should be in the range [min_cf_size, max_cf_size].
   *
   * @param f function that takes a (exclusive) minimum and (inclusive) maximum
   * CF kernel size and returns a callable object that is passed to
   * Kokkos::parallel_for()
   */
  template <typename F>
  static KOKKOS_FORCEINLINE_FUNCTION void
  cf_block_parallel_for(
    ...
    static int const min_cf_chunk_size = 16;

    auto tp =
      K::TeamPolicy<execution_space>(exec, league_size, K::AUTO)
      .set_scratch_size(0, K::PerTeam(shmem_size));
    auto block_size =
      tp.team_size_recommended(f(0, 0), K::ParallelForTag());
    if (block_size < min_cf_chunk_size) {
      // this is intended to be the implementation for OpenMP, at least
      K::parallel_for(
        kernel_name,
        K::TeamPolicy<execution_space>(exec, league_size, block_size)
        .set_scratch_size(0, K::PerTeam(shmem_size)),
        f(0, max_cf_size));
    } else {
      // this is intended to be the implementation for devices with thread
      // blocks, like CUDA
      int const max_blocks =
        std::min(
          tp.team_size_max(f(0, 0), K::ParallelForTag()),
          max_cf_size + (block_size - 1))
        / block_size;
      for (int n = (min_cf_size + (block_size - 1)) / block_size;
           n <= max_blocks;
           ++n) {
        int cfsz_min = (n - 1) * block_size;
        int cfsz_max =
  ...
  kernel(
    ...
    auto shmem_size = scratch_phscr_view::shmem_size(max_cf_size);
        ...
        [&](int cfsz_min, int cfsz_max) {
          return
            ...
            Vis<N, execution_space> vis(
              ...
              cf_radii,
              oversampling);
            auto cfsz = std::min(vis.m_cf_size[0], vis.m_cf_size[1]);
            if (do_process(vis, grid_size, {cfsz_min, cfsz_max})) {
              auto gvis =
                vg::template degrid_vis<cf_layout, grid_layout, memory_space>(
                  team_member,
                  vis,
                  cfs[vis.m_cf_grp],
                  mueller_indexes,
          ...
          [&](int cfsz_min, int cfsz_max) {
            return
              ...
              Vis<N, execution_space> vis(
                ...
                oversampling);
              if (do_process(vis, grid_size, {cfsz_min, cfsz_max})) {
                vg::template grid_vis<cf_layout, grid_layout, memory_space>(
                  team_member,
                  vis,
                  gpol,
                  cfs[vis.m_cf_grp],
          ...
          [&](int cfsz_min, int cfsz_max) {
            return
              ...
              Vis<N, execution_space> vis(
                ...
                oversampling);
              if (do_process(vis, grid_size, {cfsz_min, cfsz_max})) {
                vg::template grid_vis_no_weights<
                  cf_layout,
                  grid_layout,
                  memory_space>(
                    team_member,
